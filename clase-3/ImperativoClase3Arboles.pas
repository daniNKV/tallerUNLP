  ...
  begin
    if (a = nil) then begin
      ...
      if (a^.dato.nombre = n) then
        ...
        buscarNombre := buscarNombre(a^.hi, n) or buscarNombre(a^.hd, n);
  end;
...
begin
  ...
  cantidadSocios := contarSocios(a);

  writeln('El club cuenta con: ', cantidadSocios, ' socios');
end;

procedure InformarPromedioDeEdad(a: arbol);

  function acumularEdades(a: arbol): integer;
  begin
    if (a = nil) then
      acumularEdades := 0;
    else
      acumularEdades := a^.dato.edad + acumularEdades(a^.hi) + acumularEdades(a^.hd)

  end;
var^
  cantidadSocios: integer;
  promedioEdad: real;
  edadDeTodosLosSocios: integer;
begin
  cantidadSocios := contarSocios(a);
  edadDeTodosLosSocios := acumularEdades(a);

  promedioEdad := edadDeTodosLosSocios / cantidadSocios;

  writeln('El promedio de edad en el club es de: ', promedioEdad);

end;

procedure InformarCantidadSociosEnRango(a: arbol);

  function cantidad(a: arbol; cotaSup, cotaInf: integer): integer;
  begin
    if (a = nil) then
      cantidad := 0;
    else
      if (a^.dato.numero < cotaSuperior) and (a^.dato.numero > cotaInferior) then
        cantidad := 1 + cantidad(a^.hi, cotaSup, cotaInf) + cantidad(a^.hd, cotaSup, cotaInf)
      else 
        if(a^.dato.numero < cotaInferior) then
          cantidad := cantidad(a^.hd, cotaSup, cotaInf);
        else
          cantidad := 0 + cantidad(a^.hi, cotaSup, cotaInf);

  end;
var
  cotaSuperior, cotaInferior: integer;  
  cantidadSociosEnRango: integer;
begin
  write('Ingrese la cota superior para el número de socio: ');
  readln(cotaSuperior);
  write('Ingrese la cota inferior para el número de socio: ');
  readln(cotaInferior);

  if (cotaSuperior > cotaInferior) then begin
    cantidadSociosEnRango := cantidad(a, cotaSuperior, cotaInferior);

    writeln('La cantidad de socios con número entre ', cotaInf, ' y ', cotaSuperior, ' es de: ', cantidadSociosEnRango);
